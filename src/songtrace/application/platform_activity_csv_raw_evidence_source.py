"""Raw evidence source for local platform activity CSV files."""
...
class PlatformActivityCsvRawEvidenceSource:
    """Loads provider-neutral platform activity raw evidence from a local CSV file."""

    path: Path

    def load(self) -> tuple[RawEvidenceRecord, ...]:
        """Load platform activity records from CSV rows in deterministic order."""
        ...
        try:
            with self.path.open(newline="", encoding="utf-8") as file:
                ...
                if fieldnames is None:
                    raise ValueError("Platform activity CSV must include a header row")

                missing_fields = _REQUIRED_FIELDS.difference(fieldnames)
                if missing_fields:
                    missing = ", ".join(sorted(missing_fields))
                    raise ValueError(
                        f"Platform activity CSV is missing required field(s): {missing}"
                    )
            ...
            raise FileNotFoundError(f"Platform activity CSV file not found: {self.path}") from error


def _parse_row(row: dict[str | None, str | None], line_number: int) -> RawEvidenceRecord:
    if None in row:
        raise ValueError(f"Platform activity CSV row {line_number} has too many columns")
    ...
    signal = _required(row, "signal", line_number)
    ...
    return RawEvidenceRecord(
        ...
        reference=reference,
        signals=(_parse_signal(signal, line_number),),
        track=track,
    )
...
def _required(row: dict[str | None, str | None], field_name: str, line_number: int) -> str:
    ...
    if value is None or not value.strip():
        raise ValueError(f"Platform activity CSV row {line_number} is missing field '{field_name}'")
    return value.strip()
...
def _parse_uuid(value: str, field_name: str, line_number: int) -> UUID:
    try:
        ...
        raise ValueError(
            f"Platform activity CSV row {line_number} field '{field_name}' must be a valid UUID"
        ) from error
...
def _parse_optional_track(
    ...
    if artist is None:
        raise ValueError(
            f"Platform activity CSV row {line_number} field 'track_artist' is required "
            "when track identity is supplied"
        )
    if title is None:
        raise ValueError(
            f"Platform activity CSV row {line_number} field 'track_title' is required "
            "when track identity is supplied"
        )
...
def _parse_signal(value: str, line_number: int) -> EvidenceSignal:
    try:
        signal = EvidenceSignal(value)
    except ValueError as error:
        raise ValueError(
            f"Platform activity CSV row {line_number} field 'signal' has invalid EvidenceSignal: "
            f"{value}"
        ) from error

    if signal not in _SUPPORTED_SIGNALS:
        supported = ", ".join(sorted(signal.value for signal in _SUPPORTED_SIGNALS))
        raise ValueError(
            f"Platform activity CSV row {line_number} field 'signal' must be one of: {supported}"
        )
...
def _parse_datetime(value: str, field_name: str, line_number: int) -> datetime:
    try:
        ...
        raise ValueError(
            f"Platform activity CSV row {line_number} field '{field_name}' must be an ISO datetime"
        ) from error

    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError(
            f"Platform activity CSV row {line_number} field '{field_name}' must be timezone-aware"
        )
